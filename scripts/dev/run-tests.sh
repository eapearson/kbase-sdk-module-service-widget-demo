...
export MINIMAL_COVERAGE="${MINIMAL_COVERAGE:-65}"
echo "Running tests in path '${test_file_path}"
echo "  with MODULE_DIR of '${MODULE_DIR}'"
pytest -s --cov src/servicewidgetdemo --cov-report=html --cov-report=term --cov-report=lcov --cov-fail-under=${MINIMAL_COVERAGE} "${test_file_path}"
result_code=$?
