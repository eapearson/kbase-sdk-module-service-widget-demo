...
def test_set_config(my_config_file):
    ...
    config.config().services.ServiceWidgetDemo.url = "BAR"
    assert config.config().services.ServiceWidgetDemo.url == "BAR"

    # Set to different value, should be changed.
    config.config().services.ServiceWidgetDemo.url = "FOO"
    assert config.config().services.ServiceWidgetDemo.url == "FOO"
