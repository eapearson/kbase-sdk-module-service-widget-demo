...
import PDBReport, {PDBInfo} from "../lib/PDBReport";
...
export default class PDBTool extends Component<PDBToolProps, PDBToolState> {
    ...
    async componentDidMount() {
        ...
        try {
            ...
            const url2 = `${document.location.origin}/services/service_wizard`
            const pdbReportClient = new PDBReport({url: url2, token: this.props.token! ,timeout: 1000});
            const pdbInfos = await pdbReportClient.getPDBInfos(this.props.genomeRef);
            console.log('hey, pdb??', pdbInfos);

            // if we have tabs supplied in the props, we need to prepopulate
            // the tabs.
            const tabs = (() => {
                const propsTabs = this.props.tabs;
                if (propsTabs && propsTabs.length > 0) {
                    return pdbInfos.filter((pdbInfo) => {
                        return propsTabs.includes(pdbInfo.itemId);
                    }).map((pdbInfo) => {
                        return {
                            id: pdbInfo.itemId,
                            label: pdbInfo.structureName,
                            pdbInfo
                        }
                    });
                }
    ...
    renderTabContent(toolState: ToolState) {
        return toolState.tabs.map((tabDef) => {
            ...
            return <Tab.Pane className="flexScrollableColumn" eventKey={id} key={id}>
                ...
                <MolstarViewer structureName={tabDef.pdbInfo.structureName} format={tabDef.pdbInfo.format} />
            </Tab.Pane>
        })
    }
                            ...
                            <Tab.Pane eventKey="browser" className="flexScrollableColumn">
                                ...
                                <NarrativeContext.Consumer>
                                    {(state: NarrativeContextState) => {
                                        ...
                                        const onShowMolstar = (pdbInfo: PDBInfo) => {
                                            const processState = this.state.processState
                                            if (processState.status !== AsyncProcessStatus.SUCCESS) {
                                                return;
                                            }
                                            ...
                                            tabs.push(pdbInfo.itemId);
                                            state.value.widgetStateUpdated({
                                                widgetState: {
                                                    tabs, selectedTab: pdbInfo.itemId
                                                }
                                            });
                                            this.onShowMolstar(pdbInfo);
                                        }
                                        return <PDBBrowser
                                            pdbInfos={toolState.pdbInfos}
